...
function Output(block)   
    ...
    rudder  = block.DialogPrm(1).Data;     % wing data
...
%     AOA = delta + S_theta;                  % angle of attack on rudder
    AOA = rad2deg(delta) + rad2deg(phi) - S_theta;                  % angle of attack on rudder
    disp(AOA)           %test
    ...
    sign = (0.5-isreal(sqrt(-Cl)))*2;           %calculate Cl sighn
    Cl = sign*(abs(Cl)+0.0001);                % add 0.001 to absolute valie of Cl
    
    F_net = 0.5*sqrt(Cl^2 + Cd^2)*rho*area*Vs^2;   % force on rudder [N]
    F_theta_l = atand(Cd/Cl);                      % angle on rudder [degrees] (atan for rads) - local (oriantated to 90deg of coursw angle)
    
%     F_theta = (180 - S_theta - 90 - F_theta_1);       % angle on rudder [degrees] - global (oriantated to x and y)
    F_theta = (90 - S_theta - F_theta_l);       % angle on rudder [degrees] - global (oriantated to x and y)
...
%     Thrust = 1e6;              % [N]
    Thrust_surge = Thrust*cos(phi);      % [N]
    Thrust_sway = Thrust*sin(phi);       % [N]
    control = [(F_surge+Thrust_surge);(F_sway+Thrust_sway);T_yaw;T_roll];            % [surge,sway,roll,yaw]
