...
%     AOA = delta + S_theta;                  % angle of attack on rudder
    ...
    sign = (0.5-isreal(sqrt(-Cl)))*2;           % calculate Cl sign
    Cl = sign*(abs(Cl)+0.0001);                 % add 0.001 to absolute value of Cl
...
%     F_theta = (180 - S_theta - 90 - F_theta_1);       % angle on rudder [degrees] - global (oriantated to x and y)
    ...
    Fas = F_surge*cos(psi) + F_sway*cos(psi);          % aerodynamic side force [N]
