...
%     G(3,3) = -Kphi;           % Kphi zero with current parameters
    ...
    % Apparant and acutal wind   
    Va = sqrt(u^2+Vt^2-2*u*Vt*cos(180-Vtheta));        % to check - should u be u2 + v2? - may update equation Vtheta?
    Beta = (Vt/Va)*(180-Vtheta);
    
    xdata = [20.0,30.0,34.7,44.8,60.0,74.8,90.0,104.8,119.8,135.0,150.0,165.1,180.1];
    ydata = [0.9818,1.1957,1.2599,1.3369,1.4011,1.4182,1.4310,1.4267,1.3754,0.6995,0.4770,-0.0021,0.0021];
    x_input = Beta;     % Angle
    y_output = interp1(xdata,ydata,x_input);    %coeffent lift
    rho = 1.225;
    area = 1;   
    lift = 0.5*y_output*rho*area*Va^2;  % lift
    
    
    rudder_coefficient = 0;         % move to setUpDynamic 
%     control = [thrust;0;0;delta*rudder_coefficient*u2];  % [surge,sway,roll,yaw]
    control = [lift;0;0;delta*rudder_coefficient*u2];  % [surge,sway,roll,yaw]
