...
%     block.InputPort(1).Dimensions    = 3;    % control input vector (wind speed, wind angle, rudder angle)
    block.InputPort(1).Dimensions    = 2;    % control input vector (control, rudder angle)
...
function Derivative(block)
    ...
    % Input vector:    
    control = in(1);           % Control forces [surge,sway,roll,yaw]  [N]
...
%     control = [thrust;0;delta*rudder_coefficient*u2;0];  % [surge,sway,yaw,roll]
    control = control + [0;0;delta*rudder_coefficient*u2;0];  % [surge,sway,yaw,roll]
