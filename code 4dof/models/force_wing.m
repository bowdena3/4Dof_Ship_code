...
function setup(block)
    ...
    block.InputPort(1).Dimensions    = 1;    % control input vector (ship speed)
    % Specify whether there is direct feedthrough:
    block.InputPort(1).DirectFeedthrough = true;
    block.InputPort(1).SamplingMode  = 'Sample';
    % Size the input ports correctly:
    block.InputPort(2).Dimensions    = 2;    % control input vector (wind speed)
    % Specify whether there is direct feedthrough:
...
function Derivative(block)
    ...
    % ^replace with lift draf coeffents
    ...
    Cl_ratio = interp1(wind_oriantation,Cl_ratio_bow,W_O);
    Cd_ratio = interp1(wind_oriantation,Cd_ratio_bow,W_O);
    Cl2 = Cl_ratio*Cl1;                       % calculating 2 sail Cl
    Cd2 = Cd_ratio*Cd1;                       % calculating 2sail Cd
       
    rho = 1.225;                        % density air - move to setup?
    area = 1;                           % Sail area - move to setup?
    height = 2;                         % height of wing - move to setup?
    d = 0.5*2.5*height;                 % Distance from COG - 0.5*2.5*height?
