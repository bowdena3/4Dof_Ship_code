...
%     psi = ship_velocity(3);         % ship roll angle [rads]
    ...
    AOA = rad2deg(delta) - S_theta;                  % angle of attack on rudder % + rad2deg(psi)
...
%     Fhs = F_net*sind(F_theta+rad2deg(psi)-S_theta);          % hydrodynamic side force [N]
    ...
    % Trust propeller - Approximate thrust generated by propeller
%     Thrust_surge = Thrust*cos(psi);      % [N]
%     Thrust_sway = Thrust*sin(psi);       % [N]
    control = [(Fhr+Thrust);Fhs;T_roll;T_yaw];            % [surge,sway,roll,yaw]
