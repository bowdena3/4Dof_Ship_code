...
ine.xg = -0.46;                        % [m]             % z-coordinate of vehicle centre of gravity (-0.46)
ine.zg = -5.54;                        % [m]             % z-coordinate of vehicle centre of gravity (-3.54)
...
% Rudder parameters:
rud.rho = 1000;                        % [kg/m3] density water
...
d_bow = 0.5*2.5*(24);                 % [m] Bow sail distance from COG - 0.5*2.5*width?
d_stern = -0.5*2.5*(24);                 % [m] Stern sail distance from COG - 0.5*2.5*width?
...
wind_oriantation = [0, 20,30,35,45,60,75,90,105,120,135,150,165,180];
Cl_ratio_bow = [1.26, 1.26,1.41,1.38,1.38,1.30,1.26,1.20,1.18,1.13,0.93,0.97,-9.66,0.00];
Cd_ratio_bow = [1.32, 1.32,1.37,1.43,1.55,1.53,1.49,1.43,1.42,1.32,0.97,1.04,1.00,-0.15];
Cl_ratio_stern = [0.85, 0.85,1.00,1.10,1.08,1.10,1.13,1.15,1.21,1.14,1.17,1.11,1.07,0.00];
Cd_ratio_stern = [1.40, 1.40,1.24,1.14,1.25,1.34,1.31,1.20,1.13,1.04,1.18,1.13,1.06,1.06];
...
save('C:/Users/arthu/OneDrive/Documents/MSC Individual/code/code 4dof - PID/data/roro4dof.mat','ship');
